...
// I'm sorry but a lot of this is a mess (in terms of documentation, I think the code is pretty cool!)
// This component is used to graph a 2D shape on the page given the long/lat values
// It works pretty well, but doesn't have the same aspect ratio as the coordinates and squashes them to a square aspect ratio
class D3Shape extends React.Component {
  ...
  componentDidMount() {
    ...
    let new_coordinates = this.shiftToOrigin(
  ...
  mapToDrawableCoordinates(coordinates) {
    ...
    return coordinates.map(function (coordinate) {
      ...
      let x_final =
        x_point > x_midpoint
          ? x_point - 2 * (x_point - x_midpoint)
          : x_point + 2 * (x_midpoint - x_point);
      let y_final =
        y_point > y_midpoint
          ? y_point - 2 * (y_point - y_midpoint)
          : y_point + 2 * (y_midpoint - y_point);
